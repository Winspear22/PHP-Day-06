...
final class E05Controller extends AbstractController
{
	...
	public function vote(int $id, string $type, ManagerRegistry $doctrine, Request $request): Response
	{
		try
		{
			...
			$canLike = ($rep >= 3 || $isAdmin);
			$canDislike = ($rep >= 6 || $isAdmin);
			if ($type === 'like' && !$canLike)
			{
				$this->addFlash('error', "Il te faut au moins 3 de réputation ou être admin pour liker.");
				return $this->redirectToRoute('e01_welcome');
			}
			if ($type === 'dislike' && !$canDislike)
			{
				$this->addFlash('error', "Il te faut au moins 6 de réputation ou être admin pour disliker.");
				return $this->redirectToRoute('e01_welcome');
			}
			...
			elseif ($existing->isLike() === $isLikeRequested)
			{
				$em->remove($existing);
				$em->flush();
			}
